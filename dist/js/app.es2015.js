...
/* harmony import */ var _vue_components_Permissions_vue__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./vue_components/Permissions.vue */ "./_assets/js/vue_components/Permissions.vue");
/**
 * Vue scaffolding
 */
...
var widthM = width - margin.left - margin.right;
var heightM = height - margin.top - margin.bottom;
var allActorsObjs = [];
var links = [];
var allShows = [];
var allPeople = [];
...
function searchGraph() {
  ...
  d3.selectAll('circle[class="connNode"]').filter(function (d) {
  ...
  d3.selectAll('line[class="connLink"]').filter(function (d) {
    return d.source.name != sValue && d.target.name != sValue;
  }).style('stroke', 'grey').attr('opacity', 0.1);
  d3.selectAll('line[class="connLink"]').filter(function (d) {
    return d.source.name == sValue || d.target.name == sValue;
...
function resetGraph() {
  ...
  d3.selectAll('line[class="connLink"]').style('stroke-width', function (d) {
    return 0.5 * d.strength;
  }).style('stroke', 'black').attr('opacity', 1);
}
...
axios__WEBPACK_IMPORTED_MODULE_0___default.a.get('https://history.newtheatre.org.uk/feeds/search.json').then(function (response) {
  ...
  */
  ...
  if (fullHistory == 1) {
    ...
    allShows = resp.filter(function (x) {
      return x.type === 'show' && x.title != 'Freshers\' Fringe' && x.year_title >= '2010&ndash;11';
    });
  }
  ...
  allSeasons = rmDups(allShows.map(function (d) {
    return d.season;
  })).sort();
  allActors = rmDups(allActors).sort();
  allActors.forEach(function (a) {
    allActorsObjs.push(actorToObject(a));
  });
  allYears = rmDups(allShows.map(function (d) {
    return d.year_title;
  })).sort();

  function actorToObject(a) {
    var personShows = allShows.filter(function (d) {
      return d.cast.includes(a);
    });
    var personLinks = [];
    var personShowTitles = [];
    var earliestShow = '3000-01';
    var personRecordLink;
    var personRecord = allPeople.find(function (d) {
      return d.title == a;
    });
    ...
    personLinks = personLinks.filter(function (l) {
      return l != a;
    }).map(function (l) {
      var conns = allShows.filter(function (d) {
  ...
  var yearColourAxis = d3.scaleOrdinal().domain(allYears).range(d3.quantize(d3.interpolate('#0000ff', '#ff0000'), allYears.length));

  function addAxisLabels(xLabel, yLabel) {
    var mb = margin.bottom / 2;
    var ml = margin.left / 2;
    svg.append('text') // XLABEL
    .attr('transform', 'translate(' + width / 2 + ',' + (height - mb) + ')').style('font-weight', 'bolder').style('text-anchor', 'middle').text(xLabel);
    svg.append('text') // YLABEL
    .attr('transform', 'translate(' + (0 - ml) + ',' + height / 2 + ') rotate(-90)').attr('dy', '1em').style('font-weight', 'bolder').style('text-anchor', 'middle').text(yLabel);
  }
  /*
  *
  ...
  */
    ...
    setTimeout(function () {
      ...
      allActorsObjs.forEach(function (so) {
        so.links.forEach(function (ta) {
          if (so.name != ta.name && links.filter(function (d) {
            return d.source == ta.name && d.target == so.name || d.source == so.name && d.target == ta.name;
          }).length == 0) {
            links.push({
              source: so.name,
              target: ta.name,
      ...
      var simulation = d3.forceSimulation().nodes(allActorsObjs);
      var linkForce = d3.forceLink(links).id(function (d) {
        return d.name;
      }).strength(function (d) {
        return Math.sqrt(d.strength / 30);
      });
      var chargeForce = d3.forceManyBody().strength(-100);
      var centerForce = d3.forceCenter(width / 2, height / 2);
      simulation.force('chargeForce', chargeForce).force('centerForce', centerForce).force('links', linkForce).on('tick', tickActions); // add encompassing group for the zoom

      var g = svg.append('g').attr('class', 'everything'); // draw lines for the links

      var link = g.append('g').attr('class', 'links').selectAll('line').data(links).enter().append('line').attr('class', 'link').attr('class', 'connLink').style('stroke-width', function (d) {
        return 0.5 * d.strength;
      }).style('stroke', 'black').attr('source', function (d) {
        return d.source.name;
      }).attr('sink', function (d) {
        return d.target.name;
      }); // draw circles for the nodes

      var node = g.append('g').attr('class', 'nodes').selectAll('circle').data(allActorsObjs).enter().append('circle').attr('class', 'node').attr('class', 'connNode').attr('r', function (d) {
        return 3 * Math.sqrt(d.links.length);
      }).style('stroke-width', function (d) {
        return 0.15 * Math.sqrt(d.links.length);
      }).style('stroke', 'white').attr('fill', function (d) {
        ...
        var textWidth = d3.max([infoText, infoShows, infoLinks], function (i) {
          return i.node().getBBox().width;
        }) + 10;
        infoBox.attr('x', width - (textWidth + 5)).attr('width', textWidth).attr('height', '4.4em');
        ...
        */
        ...
        }).attr('fill', 'rgb(0,0,0)').append('svg:title').append('text').text(function (d) {
          var conns = [];
      ...
      var legendHeight = svg.attr('height') / 2.1;
      var legendWidth = 70;
      ...
      legend.append('text').attr('text-anchor', 'middle').attr('alignment-baseline', 'middle').attr('fill', 'white').attr('x', legendWidth / 2).attr('y', legendHeight - 20).text(allYears[allYears.length - 1]); // add zoom capabilities

      var zoomHandler = d3.zoom().scaleExtent([0.1, 3]).on('zoom', zoomActions);
      zoomHandler(svg); // Zoom functions
      ...
      */
      ...
      var infoBox = svg.append('rect').attr('id', 'infoBox').attr('x', width - 95).attr('y', 5).attr('width', 0).attr('height', '2.2em').attr('fill', 'white').attr('opacity', 0.5);
      var infoText = svg.append('text').attr('id', 'infoText').attr('font-size', 12).attr('font-color', 'black').attr('x', width - 10).attr('y', 5).attr('dy', '1.1em').attr('text-anchor', 'end').text('Hover over a node to see a name');
      var infoShows = svg.append('text').attr('id', 'infoShows').attr('font-color', 'black').attr('font-size', 12).attr('x', width - 10).attr('y', 5).attr('dy', '2.2em').attr('text-anchor', 'end');
      var infoLinks = svg.append('text').attr('id', 'infoLinks').attr('font-color', 'black').attr('font-size', 12).attr('x', width - 10).attr('y', 5).attr('dy', '3.3em').attr('text-anchor', 'end');
      infoBox.attr('width', infoText.node().getBBox().width + 10).attr('x', width - (infoText.node().getBBox().width + 15));
      var info = connCard.append('div').attr('id', 'info');
      info.append('input').attr('id', 'searchVal').attr('name', 'searchVal').attr('placeholder', 'Search a person').attr('type', 'text');
      info.append('button').attr('onclick', searchGraph.name).attr('class', 'btn btn-primary').append('text').text('Search');
      info.append('button').attr('onclick', resetGraph.name).attr('class', 'btn btn-secondary').append('text').text('Reset');
      /*
  ...
  if (showOtherGraphs) {
    ...
    var top10Actors = allActorsObjs.sort(function (a, b) {
    ...
    y = d3.scaleLinear().range([heightM, 0]).domain([0, d3.max(top10Actors, function (d) {
      return d.shows.length;
    })]);
    xAxis = d3.axisBottom(x);
    yAxis = d3.axisLeft(y); // Add the X Axis

    svg.append('g').attr('transform', 'translate(0,' + heightM + ')').call(xAxis).selectAll('text').attr('text-anchor', 'start').attr('transform', 'rotate(15)'); // Add the Y Axis

    svg.append('g').call(yAxis);
    svg.selectAll('bar').data(top10Actors).enter().append('rect').attr('x', function (d) {
      return x(d.name);
    }).attr('y', function (d) {
      ...
      return yearColourAxis(d.firstYear);
    }).append('svg:title').append('text').text(function (d) {
      var titles = [];
    ...
    svg.append('g').attr('transform', 'translate(0,' + heightM + ')').call(xAxis); // Add the Y Axis

    svg.append('g').call(yAxis);
    svg.selectAll('dot').data(allDatedShows).enter().append('circle').attr('r', 5).attr('cx', function (d) {
      return x(d.date);
    }).attr('cy', function (d) {
    ...
    svg.append('g').call(yAxis);
    svg.selectAll('dot').data(allShows).enter().append('circle').attr('r', 5).attr('cx', function (d) {
    ...
    addAxisLabels('Cast Size', 'Crew Size'); // SHOW NUMBERS THROUGH THE YEARS

    var showCounts = d3.nest().key(function (d) {
      return d.season;
    }).key(function (d) {
      return d.year_title;
    }).entries(allShows).sort(function (a, b) {
      return a.key - b.key;
    });
    var showCounts2 = [];
    ...
    x = d3.scaleTime().domain(d3.extent(allYears.map(function (d) {
      return yearTitleToDate(d);
    }))).range([0, widthM]);
    y = d3.scaleLinear().domain(d3.extent(showCounts2, function (d) {
      return d.values.length;
    })).range([heightM, 0]);
    allSeasons.forEach(function (s) {
      var thisSeasons = showCounts2.filter(function (d) {
        return d.season == s;
      }).sort(function (a, b) {
      ...
      linevalue = d3.line().x(function (d) {
        return x(yearTitleToDate(d.key));
      }).y(function (d) {
        return y(d.values.length);
      });
      svg.append('path').datum(thisSeasons).attr('class', 'line').attr('stroke', seasonColours(s)).attr('season', s).attr('d', linevalue);
    });
    svg.selectAll('dot').data(showCounts2).enter().append('circle').attr('season', function (d) {
      return d.season;
    ...
    svg.append('g').attr('transform', 'translate(0,' + heightM + ')').call(d3.axisBottom(x)); // Add the Y Axis
    ...
    */
    ...
    allSeasons.forEach(function (s) {
      ...
      filterCard.append('input').attr('type', 'checkbox').attr('id', s + 'Filt').attr('name', s + 'Filt').property('checked', 'true');
      filterCard.append('label').attr('for', s + 'Filt').text(s + '  ').append('div').style('width', '10px').style('height', '10px').style('display', 'inline-block').attr('width', '10px').attr('height', '10px').style('background-color', seasonColours(s)).style('border', '1px solid #000');
    }); // RESETTING

    filterCard.append('br');
    filterCard.append('button').attr('onclick', filterSeason.name).attr('class', 'btn btn-primary').append('text').text('Filter Season');
    filterCard.append('button').attr('onclick', resetFilterSeason.name).attr('class', 'btn btn-danger').append('text').text('Reset');
    /*
    *
    ...
    */
    ...
    var newNos = d3.nest().key(function (d) {
      return d.firstYear;
    ...
    y = d3.scaleLinear().domain([0, d3.max(newNos, function (d) {
      return d.values.length;
    })]).range([heightM, 0]);
    linevalue = d3.line().x(function (d) {
      return x(yearTitleToDate(d.key));
    ...
    svg.append('path').datum(newNos).attr('class', 'line').attr('stroke', 'black').attr('d', linevalue);
    svg.selectAll('dot').data(newNos).enter().append('circle').attr('r', 5).attr('fill', function (d) {
...
/***/ "./_assets/js/vue_components/Permissions.vue":
/*!***************************************************!*\
  !*** ./_assets/js/vue_components/Permissions.vue ***!
  \***************************************************/
/*! exports provided: default */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _Permissions_vue_vue_type_template_id_3530727f___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./Permissions.vue?vue&type=template&id=3530727f& */ "./_assets/js/vue_components/Permissions.vue?vue&type=template&id=3530727f&");
/* harmony import */ var _Permissions_vue_vue_type_script_lang_js___WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./Permissions.vue?vue&type=script&lang=js& */ "./_assets/js/vue_components/Permissions.vue?vue&type=script&lang=js&");
/* empty/unused harmony star reexport *//* harmony import */ var _node_modules_vue_loader_lib_runtime_componentNormalizer_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ../../../node_modules/vue-loader/lib/runtime/componentNormalizer.js */ "./node_modules/vue-loader/lib/runtime/componentNormalizer.js");
...
var component = Object(_node_modules_vue_loader_lib_runtime_componentNormalizer_js__WEBPACK_IMPORTED_MODULE_2__["default"])(
  ...
  _Permissions_vue_vue_type_template_id_3530727f___WEBPACK_IMPORTED_MODULE_0__["render"],
  _Permissions_vue_vue_type_template_id_3530727f___WEBPACK_IMPORTED_MODULE_0__["staticRenderFns"],
  false,
  null,
  null,
  null
...
/* hot reload */
if (false) { var api; }
component.options.__file = "_assets/js/vue_components/Permissions.vue"
/* harmony default export */ __webpack_exports__["default"] = (component.exports);

/***/ }),

/***/ "./_assets/js/vue_components/Permissions.vue?vue&type=script&lang=js&":
/*!****************************************************************************!*\
  !*** ./_assets/js/vue_components/Permissions.vue?vue&type=script&lang=js& ***!
  \****************************************************************************/
/*! exports provided: default */
...
/***/ "./_assets/js/vue_components/Permissions.vue?vue&type=template&id=3530727f&":
/*!**********************************************************************************!*\
  !*** ./_assets/js/vue_components/Permissions.vue?vue&type=template&id=3530727f& ***!
  \**********************************************************************************/
/*! exports provided: render, staticRenderFns */
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _node_modules_vue_loader_lib_loaders_templateLoader_js_vue_loader_options_node_modules_vue_loader_lib_index_js_vue_loader_options_Permissions_vue_vue_type_template_id_3530727f___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! -!../../../node_modules/vue-loader/lib/loaders/templateLoader.js??vue-loader-options!../../../node_modules/vue-loader/lib??vue-loader-options!./Permissions.vue?vue&type=template&id=3530727f& */ "./node_modules/vue-loader/lib/loaders/templateLoader.js?!./node_modules/vue-loader/lib/index.js?!./_assets/js/vue_components/Permissions.vue?vue&type=template&id=3530727f&");
/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "render", function() { return _node_modules_vue_loader_lib_loaders_templateLoader_js_vue_loader_options_node_modules_vue_loader_lib_index_js_vue_loader_options_Permissions_vue_vue_type_template_id_3530727f___WEBPACK_IMPORTED_MODULE_0__["render"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "staticRenderFns", function() { return _node_modules_vue_loader_lib_loaders_templateLoader_js_vue_loader_options_node_modules_vue_loader_lib_index_js_vue_loader_options_Permissions_vue_vue_type_template_id_3530727f___WEBPACK_IMPORTED_MODULE_0__["staticRenderFns"]; });
...
/***/ "./node_modules/babel-loader/lib/index.js?!./node_modules/vue-loader/lib/index.js?!./_assets/js/vue_components/Permissions.vue?vue&type=script&lang=js&":
/*!************************************************************************************************************************************************************************!*\
  !*** ./node_modules/babel-loader/lib??ref--4-0!./node_modules/vue-loader/lib??vue-loader-options!./_assets/js/vue_components/Permissions.vue?vue&type=script&lang=js& ***!
  \************************************************************************************************************************************************************************/
/*! exports provided: default */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  computed: {
    group_sums: function group_sums() {
      ...
      var sums = {
        owner: Object.keys(compGroups.owner).reduce(function (sum, key) {
          return sum + parseFloat(compGroups.owner[key] || 0);
        }, 0),
        group: Object.keys(compGroups.group).reduce(function (sum, key) {
          return sum + parseFloat(compGroups.group[key] || 0);
        }, 0),
        allUsers: Object.keys(compGroups.allUsers).reduce(function (sum, key) {
          return sum + parseFloat(compGroups.allUsers[key] || 0);
        }, 0)
      };
...
/***/ "./node_modules/vue-loader/lib/loaders/templateLoader.js?!./node_modules/vue-loader/lib/index.js?!./_assets/js/vue_components/Permissions.vue?vue&type=template&id=3530727f&":
/*!****************************************************************************************************************************************************************************************************************!*\
  !*** ./node_modules/vue-loader/lib/loaders/templateLoader.js??vue-loader-options!./node_modules/vue-loader/lib??vue-loader-options!./_assets/js/vue_components/Permissions.vue?vue&type=template&id=3530727f& ***!
  \****************************************************************************************************************************************************************************************************************/
/*! exports provided: render, staticRenderFns */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
    ...
    _c(
      ...
      [
        _vm._l(_vm.groups, function(group, name) {
          return _c(
            "div",
            { key: name, staticClass: "permissions__col" },
            [
              _c("h3", [_vm._v(_vm._s(name.toUpperCase()))]),
              _vm._v(" "),
              _vm._l(group, function(val, attr) {
                return _c("label", { key: attr }, [
                  ...
                  _c("input", {
                    ...
                    attrs: { type: "checkbox" },
                    domProps: {
                      checked: Array.isArray(group[attr])
                        ? _vm._i(group[attr], null) > -1
                        : group[attr]
                    },
                    on: {
                      change: function($event) {
                        ...
                        if (Array.isArray($$a)) {
                          var $$v = null,
                            $$i = _vm._i($$a, $$v)
                          if ($$el.checked) {
                            $$i < 0 && _vm.$set(group, attr, $$a.concat([$$v]))
                          } else {
                            $$i > -1 &&
                              _vm.$set(
                                group,
        ...
        _vm._v(" "),
        _c("div", { staticClass: "permissions__total" }, [
          _vm._v("\n      " + _vm._s(_vm.group_sums) + "\n    ")
        ])
      ],
      2
    )
...
/***/ 0:
/*!*************************************************************************************!*\
  !*** multi ./_assets/js/app.js ./_assets/js/nnt_stats.js ./_assets/sass/style.scss ***!
  \*************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

__webpack_require__(/*! /Users/jackellis/Git/gingertronmk1.github.io/_assets/js/app.js */"./_assets/js/app.js");
__webpack_require__(/*! /Users/jackellis/Git/gingertronmk1.github.io/_assets/js/nnt_stats.js */"./_assets/js/nnt_stats.js");
