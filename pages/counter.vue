...
<template>
  <div
    class="flex flex-col md:flex-row flex-wrap gap-4 *:flex *:flex-col *:min-w-[33%] *:flex-1 *:gap-y-4 [&_h3]:text-2xl"
  >
    <div>
      <h3>Counter</h3>
      <AppButton @click="countUp()" v-text="`+${increment}`" />
      <AppInput
        v-model="count"
        type="text"
        pattern="\d"
        ...
        class="text-2xl"
      />
      <AppButton @click="countDown()" v-text="`-${increment}`" />
      <AppButton class="bg-red-600" @click="resetCounter()" v-text="`RC`" />
    </div>
    <div>
      <h3>Increment</h3>
      ...
      <AppInput
        ...
        class="text-2xl"
      />
      <AppButton @click="incrementDown()" v-text="`-`" />
      <AppButton class="bg-red-600" @click="resetIncrement()" v-text="`RI`" />
    </div>
    <div class="">
      <h3>Target</h3>
      <AppInput
